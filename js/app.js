...
function resetSearchArea() {
    ...
    try {
        ...
        // 更新页面标题
        document.title = `RunTV影城 - 皇后大道東8964號`;
    } catch (e) {
        console.error('您的瀏覽器boomer辣:', e);
    }
}
...
async function importConfigFromUrl() {
    ...
    document.getElementById('confirmUrlImport').addEventListener('click', async () => {
        ...
        try {
            ...
            if (config.name !== 'RunTV-Settings') throw '配置文件格式不正确';
...
async function exportConfig() {
    ...
    config['name'] = 'RunTV-Settings';  // 配置文件名，用于校验
    ...
    // 将配置数据保存为 JSON 文件
    saveStringAsFile(JSON.stringify(config), 'RunTV-Settings_' + times + '.json');
}
